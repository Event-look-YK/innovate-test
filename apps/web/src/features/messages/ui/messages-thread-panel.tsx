import { Button } from "@innovate-test/ui/components/button";
import { Input } from "@innovate-test/ui/components/input";
import { ScrollArea } from "@innovate-test/ui/components/scroll-area";
import { Separator } from "@innovate-test/ui/components/separator";
import { cn } from "@innovate-test/ui/lib/utils";
import { ClockIcon } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

import { useSendMessage } from "@/features/messages/hooks/use-messages";
import { useCurrentUser } from "@/shared/hooks/use-current-user";
import type { ThreadMessage } from "@/shared/types/message";

type Props = {
  threadId: string;
  messages: ThreadMessage[] | undefined;
};

export const MessagesThreadPanel = ({ threadId, messages }: Props) => {
  const [messageBody, setMessageBody] = useState("");
  const sendMessage = useSendMessage();
  const { user } = useCurrentUser();

  const handleSend = async () => {
    const body = messageBody.trim();
    if (!body) return;
    try {
      await sendMessage.mutateAsync({ threadId, body });
      setMessageBody("");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to send message");
    }
  };

  return (
    <div className="flex flex-col">
      <ScrollArea className="h-[min(420px,50vh)] p-4 md:h-auto md:max-h-[calc(100vh-220px)]">
        <div className="flex flex-col gap-3">
          {messages?.map((m) => (
            <div
              key={m.id}
              className={cn(
                "max-w-[85%] rounded-lg border border-border px-3 py-2 text-sm",
                m.authorId === user?.id ? "ml-auto bg-primary/10" : "bg-muted/50",
              )}
            >
              <p className="text-xs text-muted-foreground">{m.author}</p>
              <p>{m.body}</p>
              <p className="mt-1 flex items-center gap-1 text-[10px] text-muted-foreground">
                {m.status === "sent" ? <ClockIcon className="size-3" /> : null}
                {new Date(m.sentAt).toLocaleString("uk-UA", {
                  dateStyle: "short",
                  timeStyle: "short",
                })}
              </p>
            </div>
          ))}
        </div>
      </ScrollArea>
      <Separator />
      <div className="flex gap-2 p-3">
        <Input
          className="flex-1"
          placeholder="Message..."
          value={messageBody}
          onChange={(e) => setMessageBody(e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              e.preventDefault();
              void handleSend();
            }
          }}
        />
        <Button disabled={sendMessage.isPending} type="button" onClick={() => void handleSend()}>
          {sendMessage.isPending ? "Sending..." : "Send"}
        </Button>
      </div>
    </div>
  );
};
